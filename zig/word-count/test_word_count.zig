...
//
// test "default from example" {
//     const s = "That's the password: 'PASSWORD 123'!\", cried the Special Agent.\nSo I fled.";
//     var map = try countWords(testing.allocator, s);
//     defer freeKeysAndDeinit(&map);
//     try testing.expectEqual(@as(u32, 10), map.count());
//     try testing.expectEqual(@as(?u32, 1), map.get("that's"));
//     try testing.expectEqual(@as(?u32, 1), map.get("the"));
//     try testing.expectEqual(@as(?u32, 2), map.get("password"));
//     try testing.expectEqual(@as(?u32, 1), map.get("123"));
//     try testing.expectEqual(@as(?u32, 1), map.get("cried"));
//     try testing.expectEqual(@as(?u32, 2), map.get("the"));
//     try testing.expectEqual(@as(?u32, 1), map.get("special"));
//     try testing.expectEqual(@as(?u32, 1), map.get("agent"));
//     try testing.expectEqual(@as(?u32, 1), map.get("so"));
//     try testing.expectEqual(@as(?u32, 1), map.get("i"));
//     try testing.expectEqual(@as(?u32, 1), map.get("fled"));
// }
//
test "count one word" {
    const s = "word";
