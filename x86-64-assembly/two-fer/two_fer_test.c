...
void test_short_name_given(void) {
    // TEST_IGNORE();
    char buffer[BUFFER_SIZE];

    two_fer("A", buffer);
    TEST_ASSERT_EQUAL_STRING("One for A, one for me.", buffer);
}

void test_long_name_given(void) {
    // TEST_IGNORE();
    char buffer[BUFFER_SIZE];

    two_fer("Alexander", buffer);
    TEST_ASSERT_EQUAL_STRING("One for Alexander, one for me.", buffer);
}
...
int main(void) {
    ...
    RUN_TEST(test_short_name_given);
    RUN_TEST(test_long_name_given);
    return UNITY_END();
}
